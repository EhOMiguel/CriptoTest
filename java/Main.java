...
public class Main{
  ...
  public static void main(String[] args) {
    ...
    BigInteger alfa = null;
    BigInteger segredo = null;
    ...
    pattern = Pattern.compile("primo:\\s*(\\d+)");
    matcher = pattern.matcher(response);
    if (matcher.find()) {
        System.out.println("primo = "+matcher.group(1));
        primo = new BigInteger(matcher.group(1));
    }

    pattern = Pattern.compile("inteiro:\\s*(\\d+)");
    matcher = pattern.matcher(response);
    if (matcher.find()) {
        System.out.println("inteiro = "+ matcher.group(1));
